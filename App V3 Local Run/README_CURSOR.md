...
This file orients automated coding tools to the **App V3 Local Run** tree: multi-agent Shiny app, MCP tool registry, and BM25 RAG over FAA-style documents. For Posit Connect deployment scripts, see **`App V3 For Deployment/`**.
...
All paths are relative to **`App V3 Local Run/`**.
...
| [`app/core/config.py`](app/core/config.py) | Loads `.env` from **this app folder** (`App V3 Local Run`), Ollama and MCP settings. |
...
Copy [`.env.example`](.env.example) to `.env` in **`App V3 Local Run`**. Never commit secrets. Key names: `OLLAMA_API_KEY`, `OLLAMA_HOST`, `OLLAMA_MODEL`, `MCP_BASE_URL`, `RAG_DATA_DIR`, `TSA_WAIT_TIMES_PROXY_URL`, `OPENSKY_CLIENT_ID`, `OPENSKY_CLIENT_SECRET`.
