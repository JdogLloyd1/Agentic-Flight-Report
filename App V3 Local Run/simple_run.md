...
Use these steps from the **`App V3 Local Run`** folder (the directory that contains `app/` and `mcp_server/`).
...
To run the MCP SSE server anyway (for example to test integrations that expect a remote MCP):
...
To force the Shiny workflow to call tools over HTTP instead of in-process, set **`MCP_BASE_URL`** in `.env` to the base URL of an HTTP tool server that exposes **`POST /tools/call`** (see the sibling **`App V3 For Deployment`** package for publishing that bridge to Posit Connect).
