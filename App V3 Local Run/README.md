# Airspace Intelligence Agent — App V3 (local)
...
This folder is for **local development and testing** on your machine. For **Posit Connect** deployment, use the sibling **[`App V3 For Deployment/`](../App%20V3%20For%20Deployment/)** tree.
...
When `MCP_BASE_URL` is unset, Agent 1 calls the same tool implementations **in process** from `mcp_server.tools` (no separate server required for local runs). Set `MCP_BASE_URL` to point at a remote HTTP tool bridge if you need tools to run outside the Shiny process.
...
From the **`App V3 Local Run`** directory (this folder):
...
| `MCP_BASE_URL` | If set, tools are invoked over HTTP against your tool bridge; if empty, tools run in-process. |
...
Open **http://127.0.0.1:8000** (override with `SHINY_HOST` / `SHINY_PORT`). Optional MCP server (SSE):
