# search.py — BM25 keyword search over ingested FAA chunks (rank_bm25).
# Derived from App V3 Local Run/app/rag/search.py.
...
def _load_index() -> tuple[list[dict[str, Any]], BM25Okapi | None]:
    ...
    if _INDEX is not None:
        return _INDEX
    path = Path(config.RAG_DATA_DIR) / "index" / "chunks.json"
    if not path.is_file():
        _INDEX = ([], None)
        return _INDEX
    try:
...
def _load_airport_index() -> dict[str, list[str]]:
    ...
    path = Path(config.RAG_DATA_DIR) / "index" / "airport_index.json"
    if not path.is_file():
        _AIRPORT_INDEX = {}
        return _AIRPORT_INDEX
    try:
...
def format_reference_hits(hits: list[dict[str, Any]]) -> str:
    ...
    cap = max(500, config.RAG_HIT_CONTENT_CHARS)
    lines: list[str] = []
    for h in hits:
        how = "structured (airport index)" if h.get("retrieval") == "structured" else "bm25"
        raw = str(h.get("content", "") or "")
        body = raw[:cap] if len(raw) <= cap else raw[:cap] + f"\n... [truncated {len(raw) - cap} chars]"
        lines.append(
