# data_collector.py — Agent 1 (tool-calling live NAS / weather / TFR / TSA).
# Derived from App V3 Local Run/app/agents/data_collector.py.
...
def run_data_collector(task: str, model: str | None = None) -> str:
    ...
    config.log_info(f"Agent1 data_collector: start task_chars={len(task)}")
    tools = mcp_client.list_tool_schemas()
    config.log_info(f"Agent1 data_collector: loaded {len(tools)} tool schema(s)")
    out = ollama_client.agent_run(ROLE, task, tools=tools, model=model)
    config.log_info(f"Agent1 data_collector: end out_chars={len(out)}")
    return out
